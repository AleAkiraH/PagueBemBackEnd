...
This repository includes a Dockerfile that builds an Amazon Linux 2 based image with native libzbar and the Python dependencies installed so you can test the Lambda handler locally.
...
- To run the Flask test server (for manual HTTP testing), override the entrypoint: `docker run --rm -p 8080:8080 --entrypoint python3 paguebem/qrreader:lambda server.py`.

Notes
- The image installs and registers libzbar so `pyzbar` works inside the container.
- `opencv-python-headless` may require additional system libraries (we include common ones in the Dockerfile). If you encounter import errors for cv2, check container logs and add the needed system packages.
